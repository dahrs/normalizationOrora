...
def unifyListOfTestSetsIntoOne(listOfTestFiles, outputUnifiedFilePath=None):
	...
	dataframes = [ utilsOs.getDataFrameFromArgs(dfPath) for dfPath in listOfTestFiles ]
	#concatenate them
	unifiedDf = pd.concat(dataframes)
	#dump the result
	if outputUnifiedFilePath != None:
		utilsOs.dumpDataFrame(unifiedDf, outputUnifiedFilePath)
...
applyEvaluator(u'./006transformed/transformedTest2.tsv', u'./002sets/test2GS.tsv', u'./008result/testResults2.tsv', verbose)
applyEvaluator(u'./006transformed/transformedTest3.tsv', u'./002sets/test3GS.tsv', u'./008result/testResults3.tsv', verbose)
